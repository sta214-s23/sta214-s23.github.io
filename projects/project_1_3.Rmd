...
**Due:** Monday, March 20, 12:00pm (noon). Submit *both* your knitted HTML file and the R Markdown file to Canvas.

**Collaboration:** This project *must* be completed independently. See "Collaboration" below for more details.

# Overview

In the first part of this project, you began to explore the research questions with exploratory data analysis for the relevant variables. In the second part of this project, you fit a model to address the **first** research question, performed model diagnostics, and conducted a hypothesis test. In the final part of the project, you will assess the predictive ability of different models to predict whether survey respondents exercise regularly. You will then combine your analysis with work from parts I and II, to create a full report.
...
2. How well can we predict whether a patient exercises regularly, using the variables available in the data?
...
For the second research question, we are interested in prediction. The model for research question 1 may or may not be appropriate for this prediction question, so you must explore more than one model. Use forward stepwise selection to choose a model from the available variables, then compare this model with your model from question 1.

To assess the predictive abilities of your two models, report metrics like accuracy, sensitivity, and specificity. You should also create an ROC curve for each model, and report the AUCs. Make sure to clearly explain to your client how well this model each model performs at the task of prediction.
