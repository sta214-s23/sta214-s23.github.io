...
```r
music <- read_csv("https://sta214-s23.github.io/slides/musicdata.csv")
```

# Parametric bootstrapping

Parametric bootstrapping can be used to test hypotheses, without having to know the distribution of the test statistic. In this assignment, you will perform a hypothesis test with the likelihood ratio statistic, but you will compare the test statistic to a parametric bootstrap distribution instead of the $\chi^2$ distribution.
...
In this assignment, you will test whether there is a difference in anxiety between large and small ensemble performances, after accounting for audience type, and allowing for variation in anxiety between musicians.
...
We will use the reduced model to simulate data, so report the variance estimates $\widehat{\sigma}_u^2$ and $\widehat{\sigma}_\varepsilon^2$ for the reduced model.
:::

:::{.question}
#### Question 3

Calculate a test statistic on the *observed* data. For this test, we will use the test statistic (but not the p-value!) from a likelihood ratio test. If you saved your full and reduced models from Question 2 as `m1` and `m0`, respectively, then you can calculate the test statistic by
```{r eval=F}
...
Using a for loop, repeat steps 2 -- 3 500 times, and store the results in a vector called `null_stats`. Here is some code to get you started:
...
for(sim in ...){
  ...
  # remember to save the results in null_stats
}
```
:::
