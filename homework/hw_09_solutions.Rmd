...
**Grading:** 1 pt for noting the excess 0s and connecting the excess 0s to a zero-inflated model; 1 pt for the latent variable. It is possible that students will propose a different latent variable, which is ok as long as the latent variable is reasonable. I think the most natural latent variable here is whether or not they go fishing.
...
**Grading:** Answers may vary somewhat here. The point is that people who visit for longer have more opportunity to catch fish.
...
**Grading:** Lose 1 pt if missing/incorrect random component. Lose 1 pt if missing/incorrect logistic or Poisson parts of the ZIP model. Lose 1 pt for incorrect notation.
...
**Grading:** 1 pt for logistic part, 1 pt for Poisson part. It is ok if they interpret on the log-odds or log-mean scale.
...
**Grading:** 1 pt for each confidence interval.
...
**Grading:** 1 pt for calculating $\alpha$, 1 pt for calculating $\lambda$, 1 pt for correctly combining them. 

Some potential errors: students might calculate $P(Y_i = 1)$ instead of $P(Y_i \geq 1)$; students might misinterpret $\widehat{\alpha}_i$ as the probability of catching fish, rather than the probability of not going fishing.
...
**Grading:** Lose 1 pt if testing the wrong part of the ZIP model. Lose 1 pt if testing the wrong direction for the alternative hypothesis. Lose 1 pt if they use a LRT instead of a Wald test.
...
**Grading:** 1 pt for running code, 1 pt for comparing estimates to true values.
...
**Grading:** Lose 1 pt for using the wrong part of the ZIP model.
...
**Grading:** If their coverage values are substantially different from these numbers, something has gone wrong. Coverage for the Poisson should be much lower than for the ZIP model, and coverage for the ZIP model should be around 95%. 
...
**Grading:** It is ok if they use different values of $\gamma_0$, provided they choose a range of values. However, the general pattern should be the same as the results shown here; if not, something has probably gone wrong with their code.

They don't have to use a nested for loop.
